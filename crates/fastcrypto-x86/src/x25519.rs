...
global_asm!(
    include_str!("x25519/curve25519_x25519.s"),
    options(att_syntax)
);
global_asm!(
    include_str!("x25519/curve25519_x25519_alt.s"),
    options(att_syntax)
);
global_asm!(
    include_str!("x25519/curve25519_x25519base.s"),
    options(att_syntax)
);
global_asm!(
    include_str!("x25519/curve25519_x25519base_alt.s"),
    options(att_syntax)
);

unsafe extern "C" {
    /// s2n-bignum `curve25519_x25519_byte`, BMI2 + ADX.
    fn fastcrypto_curve25519_x25519_byte(res: *mut u8, scalar: *const u8, point: *const u8);
    /// s2n-bignum `curve25519_x25519_byte_alt`, baseline x86_64.
    fn fastcrypto_curve25519_x25519_byte_alt(res: *mut u8, scalar: *const u8, point: *const u8);
    /// s2n-bignum `curve25519_x25519base_byte`, BMI2 + ADX.
    fn fastcrypto_curve25519_x25519base_byte(res: *mut u8, scalar: *const u8);
    /// s2n-bignum `curve25519_x25519base_byte_alt`, baseline x86_64.
    fn fastcrypto_curve25519_x25519base_byte_alt(res: *mut u8, scalar: *const u8);
}
...
pub fn x25519(out: &mut [u8; 32], scalar: &[u8; 32], point: &[u8; 32]) {
    ...
    unsafe {
        match variant() {
            Variant::Adx => {
                fastcrypto_curve25519_x25519_byte(
                    out.as_mut_ptr(),
                    scalar.as_ptr(),
                    point.as_ptr(),
                );
            ...
            Variant::Baseline => {
                fastcrypto_curve25519_x25519_byte_alt(
                    out.as_mut_ptr(),
                    scalar.as_ptr(),
                    point.as_ptr(),
                );
...
pub fn x25519_base(out: &mut [u8; 32], scalar: &[u8; 32]) {
    ...
    unsafe {
        match variant() {
            Variant::Adx => {
                fastcrypto_curve25519_x25519base_byte(out.as_mut_ptr(), scalar.as_ptr());
            }
            Variant::Baseline => {
                fastcrypto_curve25519_x25519base_byte_alt(out.as_mut_ptr(), scalar.as_ptr());
            }
        }
    }
}
...
pub unsafe fn x25519_adx(out: &mut [u8; 32], scalar: &[u8; 32], point: &[u8; 32]) {
    // SAFETY: pointer contract as in `x25519`; the caller guarantees BMI2/ADX.
    unsafe { fastcrypto_curve25519_x25519_byte(out.as_mut_ptr(), scalar.as_ptr(), point.as_ptr()) }
}
...
pub fn x25519_baseline(out: &mut [u8; 32], scalar: &[u8; 32], point: &[u8; 32]) {
    ...
    unsafe {
        fastcrypto_curve25519_x25519_byte_alt(out.as_mut_ptr(), scalar.as_ptr(), point.as_ptr());
    }
}
...
///
/// The CPU must support BMI2 and ADX.
pub unsafe fn x25519_base_adx(out: &mut [u8; 32], scalar: &[u8; 32]) {
    // SAFETY: pointer contract as in `x25519_base`; the caller guarantees
    // BMI2/ADX.
    unsafe { fastcrypto_curve25519_x25519base_byte(out.as_mut_ptr(), scalar.as_ptr()) }
}

/// Computes the X25519 public key with the baseline-x86_64 routine.
///
/// Testing and benchmarking only. Executable on every x86_64 CPU.
pub fn x25519_base_baseline(out: &mut [u8; 32], scalar: &[u8; 32]) {
    // SAFETY: pointer contract as in `x25519_base`. The `_alt` routine uses
    // only baseline x86_64 instructions.
    unsafe { fastcrypto_curve25519_x25519base_byte_alt(out.as_mut_ptr(), scalar.as_ptr()) }
}
...
mod tests {
    ...
    ///
    /// Requires a C preprocessor. That is a test-only dependency: the crate
    /// itself builds from the committed `.s` with no build script and no C
    /// toolchain.
    #[test]
    fn regenerating_the_assembly_reproduces_it() {
        ...
        let root = format!("{}/src/x25519", env!("CARGO_MANIFEST_DIR"));
        ...
        for unit in UNITS {
            let output = Command::new("cpp")
                ...
                .output()
                .expect("a C preprocessor is required to verify the vendored assembly");
            assert!(
    ...
    fn vendored_upstream_matches_the_recorded_digests() {
        ...
        let root = format!("{}/src/x25519/upstream", env!("CARGO_MANIFEST_DIR"));
        for (name, expected) in DIGESTS {
            let bytes = fs::read(format!("{root}/{name}")).expect("vendored upstream file");
            let mut digest = fastcrypto_core::sha256::Sha256::new();
            digest.update(&bytes);
            assert_eq!(
                hex(&digest.finalize()),
                *expected,
